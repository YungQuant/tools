...
ticker, quantity, minV, ovAgg, depth = args[1], float(args[2]), float(args[3]), int(args[4]), int(args[5])
if ticker[-3:] == "BTC":
    mtu = 0.00000001
    mtu2 = 0.00000002
elif ticker[-3:] == "ETH":
...
while(1):
    try:
        ...
        bVol, aVol = sum([float(order[2]) for order in orders['BUY'][:depth]]), sum([float(order[2]) for order in orders['SELL'][:depth]])
        print("Kucoin AutoSellBid Version 3c -yungquant")
        print("Ticker:", ticker, "sQuantity:", sQuantity, "Quantity:", quantity, "minV:", minV, "ovAgg:", ovAgg, "depth:", depth)
        print("sBals:", sBals, "bals:", filterBalances(client.get_all_balances()))
        print("starttime:", starttime, "time:", timeStr)
        print("bVol:", bVol, "aVol:", aVol)
        if bVol > minV:
            ...
            print(client.create_sell_order(ticker, str(bid - (mtu2 * depth)), str(bVol/bid)[:7]))
            quantity -= bVol
