...
#icker, quantity, aggression, window, ovAgg = args[1], float(args[2]), float(args[3]), float(args[4]). float(args[5])
...
while(1):
    try:
        ...
        if len(spreads) > window and timeCnt % ovAgg == 0:
            spreadStd = np.std(spreads[-int(window):])
            spreadMean = np.mean(spreads[-int(window)])
            if spread > spreadMean + (spreadStd * aggression):
                ...
                if quantity <= aVol:
                    ...
                    exit(code=0)
                print("client.create_buy_order(", ticker, ask, aVol, ")")
                print(client.create_buy_order(ticker, str(ask), str(aNatVol)))
                quantity -= aVol
